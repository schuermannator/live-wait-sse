...
use rocket::tokio::stream::Stream;
...
impl Message {
    ...
    pub fn data<S: Into<String>>(data: S) -> Self {
        Self { event: None, id: None, data: Some(data.into()), retry: None }
    }
...
impl<S: Stream<Item=Message>> EventSource<S> {
    ...
    pub fn from_stream(stream: S) -> EventSource<S> {
        EventSource { stream, state: State::Reading(Cursor::new(vec![])) }
    }
}
                ...
                State::Reading(cursor) => {
                    if cursor.position() as usize >= cursor.get_ref().len() {
                        ...
                        match this.stream.as_mut().poll_next(cx) {
                            ...
                            Poll::Ready(Some(next_event)) => *this.state = State::Reading(Cursor::new(next_event.serialize())),
